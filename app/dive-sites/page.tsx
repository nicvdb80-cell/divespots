    ...
    <main style={{minHeight:'100vh',background:'#fff'}}>
      ...
      <div style={{background:'linear-gradient(135deg,#0a1628,#1e3a5f)',padding:'3rem 2rem',color:'#fff',borderBottom:'1px solid #1e3a5f'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          ...
          <p style={{color:'#64748b',fontSize:14,lineHeight:1.7,maxWidth:560}}>Browse all verified dive sites by country, area, type, and difficulty. Currently featuring Bali, Indonesia.</p>
        </div>
      </div>
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'2.5rem 2rem'}}>
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:'2rem'}}>
          <div style={{padding:'6px 14px',background:'#0a1628',color:'#fff',borderRadius:8,fontSize:12,fontWeight:700,cursor:'pointer'}}>All areas</div>
          {BALI_AREAS.map(a=>(
            <div key={a.slug} style={{padding:'6px 14px',border:'1px solid #e2e8f0',color:'#64748b',borderRadius:8,fontSize:12,fontWeight:500,cursor:'pointer'}}>{a.name} <span style={{color:'#94a3b8',fontSize:11}}>({a.count})</span></div>
          ))}
        </div>
