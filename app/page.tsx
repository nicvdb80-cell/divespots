...
export default function Home() {
  const featured = BALI_SITES.filter(s => ['usat-liberty-wreck','manta-point','crystal-bay','menjangan-wall','secret-bay-gilimanuk'].includes(s.slug))
  const countries = [
  ...
  return (
    <main style={{minHeight:'100vh',background:'#fff'}}>
      ...
      {/* Hero */}
      <div style={{background:'linear-gradient(135deg,#0a1628 0%,#0d2845 40%,#1a3a5c 100%)',padding:'6rem 2rem 5rem',color:'#fff',textAlign:'left',maxWidth:'100%',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:1100,margin:'0 auto',position:'relative'}}>
          ...
          <div style={{display:'flex',gap:12,marginBottom:'4rem'}}>
            <Link href="/indonesia/bali/tulamben/usat-liberty-wreck" style={{padding:'14px 32px',background:'#ef4444',color:'#fff',borderRadius:8,fontWeight:800,fontSize:14,letterSpacing:1,textTransform:'uppercase'}}>Explore Bali</Link>
      ...
      <div style={{background:'#f8fafc',padding:'4rem 2rem'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',marginBottom:'2rem'}}>
            ...
            <Link href="/map" style={{fontSize:13,color:'#2563eb',fontWeight:500}}>View all on map →</Link>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>
            {countries.map(c=>(
              <Link key={c.slug} href={c.live?`/${c.slug}`:`/${c.slug}`}
                style={{background:c.live?'linear-gradient(135deg,#0a1628,#1e3a5f)':'#fff',borderRadius:16,padding:'1.75rem',color:c.live?'#fff':'#0a1628',border:c.live?'none':'1px solid #e2e8f0',display:'block',position:'relative',overflow:'hidden'}}>
                <div style={{position:'absolute',top:12,left:12}}>
                  <span style={{fontSize:10,padding:'3px 10px',borderRadius:20,fontWeight:700,background:c.live?'#22c55e':'#e2e8f0',color:c.live?'#fff':'#94a3b8'}}>
                ...
                <div style={{marginTop:28}}>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                    ...
                    <span style={{fontSize:12,color:c.live?'#60a5fa':'#2563eb',fontWeight:600}}>{c.live?'Explore →':'Learn more →'}</span>
                  </div>
                </div>
              </Link>
            ))}
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:12}}>
            {featured.map(site=>(
              <Link key={site.slug} href={`/indonesia/bali/${site.areaSlug}/${site.slug}`}
                style={{background:'linear-gradient(135deg,#0a1628,#1e3a5f)',borderRadius:12,padding:'1.25rem',color:'#fff',display:'block'}}>
                <div style={{fontSize:10,padding:'2px 8px',background:'rgba(255,255,255,0.1)',borderRadius:10,color:'#94a3b8',display:'inline-block',marginBottom:8}}>{site.type}</div>
