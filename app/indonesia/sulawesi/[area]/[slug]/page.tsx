...
export default async function SulawesiDiveSitePage({ params }: { params: Promise<{ area: string; slug: string }> }) {
  ...
  const sidebarSites = SULAWESI_SITES.filter(s => s.areaSlug === site.areaSlug)
      ...
      <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", minHeight: "calc(100vh - 60px)" }}>
        <aside style={{ background: "#0a1628", borderRight: "1px solid #1e3a5f", overflowY: "auto", maxHeight: "calc(100vh - 60px)", position: "sticky", top: 60 }}>
          ...
          <div style={{ borderTop: "1px solid #1e3a5f", padding: "1rem" }}>
            <Link href="/indonesia/sulawesi" style={{ display: "block", fontSize: 12, color: "#60a5fa", marginBottom: "1rem" }}>← All Sulawesi sites</Link>
          </div>
        </aside>
        ...
        <div style={{ overflowY: "auto" }}>
          <div style={{ background: "#020d1a", padding: "1.25rem 2rem", borderBottom: "1px solid #1e3a5f" }}>
            <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 4 }}>
              <Link href="/" style={{ color: "#475569" }}>Home</Link> › <Link href="/indonesia" style={{ color: "#475569" }}>Indonesia</Link> › <Link href="/indonesia/sulawesi" style={{ color: "#475569" }}>Sulawesi</Link> › {site.area}
            </div>
          ...
          <div style={{ padding: "1.5rem 2rem", display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.5rem", alignItems: "start" }}>
            <div>
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 6 }}>Indonesia › Sulawesi › {site.area}</div>
            ...
            <div>
              <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "1.25rem", marginBottom: 12 }}>
                ...
                {([["Location", site.area + ", Sulawesi"], ["Type", site.type], ["Access", site.access], ["Depth", site.minDepth + "m – " + site.maxDepth + "m"], ["Visibility", site.visibility], ["Water temp", site.temp], ["Difficulty", site.difficulty], ["Best time", site.bestTime]] as [string,string][]).map(([k, v]) => (
              ...
              {nearby.length > 0 && (
                <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "1.25rem" }}>
                  ...
                  {nearby.map(s => (
                    <Link key={s.slug} href={`/indonesia/sulawesi/${s.areaSlug}/${s.slug}`} style={{ display: "flex", gap: 10, padding: "8px 0", borderBottom: "1px solid #f8fafc", alignItems: "center", textDecoration: "none" }}>
