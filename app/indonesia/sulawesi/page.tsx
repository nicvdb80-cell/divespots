  ...
  return (
    <main style={{ minHeight:'100vh', background:'#0d1b2e' }}>
      ...
      <div style={{ background:'#020d1a url(/hero-bg.png) center center / cover no-repeat', padding:'2.5rem 2rem', color:'#fff', borderBottom:'1px solid #1e3a5f' }}>
        <div style={{ fontSize:11,color:'#60a5fa',marginBottom:8 }}>🇮🇩 Indonesia · Sulawesi</div>
        <h1 style={{ fontSize:36,fontWeight:900,marginBottom:10 }}>SULAWESI DIVE SITES</h1>
        <p style={{ color:'#64748b',maxWidth:560,fontSize:13,lineHeight:1.7,marginBottom:'1.5rem' }}>Sulawesi is a diver&apos;s world in itself — from the legendary vertical walls of Bunaken teeming with turtles, to the black sand of Lembeh Strait, the world capital of muck diving. Add pelagic seamounts off Bangka Island and you have one of the most diverse dive destinations on the planet.</p>
      ...
      <div style={{ maxWidth:1100,margin:'0 auto',padding:'2rem',display:'grid',gridTemplateColumns:'240px 1fr',gap:'2rem' }}>
        <div>
          <div style={{ background:'#0a1628',border:'1px solid #1e3a5f',borderRadius:12,padding:'1.25rem',marginBottom:12 }}>
            ...
            {SULAWESI_AREAS.map(a=>(
              <Link key={a.slug} href={`/indonesia/sulawesi/areas/${a.slug}`} style={{ padding:'8px 0',borderBottom:'1px solid #1e3a5f',display:'flex',justifyContent:'space-between',alignItems:'center',textDecoration:'none',cursor:'pointer' }}>
                <div>
        ...
        <div>
          <h2 style={{ fontSize:18,fontWeight:800,color:'#fff',marginBottom:'1.25rem' }}>TOP DIVE SITES IN SULAWESI</h2>
          <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
            {SULAWESI_SITES.map(site=>(
              <Link key={site.slug} href={`/indonesia/sulawesi/${site.areaSlug}/${site.slug}`} style={{ display:'flex',alignItems:'center',gap:14,padding:'0.9rem 1.25rem',background:'#0a1628',border:'1px solid #1e3a5f',borderRadius:10, textDecoration:'none' }}>
