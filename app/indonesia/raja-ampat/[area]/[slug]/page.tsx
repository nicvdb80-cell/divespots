import { RAJA_AMPAT_SITES, getRajaAmpatSiteBySlug, RAJA_AMPAT_AREAS } from '@/lib/data'
...
export async function generateMetadata({ params }: { params: Promise<{ area: string; slug: string }> }) {
  ...
  const site = getRajaAmpatSiteBySlug(slug)
  if (!site) return {}
  return {
    title: `${site.name} Dive Site Guide | ${site.area}, Raja Ampat, Indonesia | Dive Spots`,
    description: `Complete diver guide to ${site.name} in ${site.area}, Raja Ampat. Depth ${site.minDepth}-${site.maxDepth}m.`,
  }
}

export default async function RajaAmpatDiveSitePage({ params }: { params: Promise<{ area: string; slug: string }> }) {
  const { slug } = await params
  const site = getRajaAmpatSiteBySlug(slug)
  if (!site) { notFound(); return null }
  const nearby = RAJA_AMPAT_SITES.filter(s => s.areaSlug === site.areaSlug && s.slug !== site.slug).slice(0, 3)
      ...
      <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", minHeight: "calc(100vh - 60px)" }}>
        <aside style={{ background: "#0a1628", borderRight: "1px solid #1e3a5f", overflowY: "auto", maxHeight: "calc(100vh - 60px)", position: "sticky", top: 60 }}>
          <div style={{ padding: "1rem", borderBottom: "1px solid #1e3a5f" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 800, color: "#fff" }}>RAJA AMPAT</div>
            <div style={{ fontSize: 10, color: "#475569", marginTop: 2 }}>Top 10 dive sites in Raja Ampat</div>
          </div>
          <div style={{ padding: "0.5rem 0" }}>
            {RAJA_AMPAT_SITES.map(s => (
              <Link key={s.slug} href={`/indonesia/raja-ampat/${s.areaSlug}/${s.slug}`}
          ...
          <div style={{ borderTop: "1px solid #1e3a5f", padding: "1rem" }}>
            <Link href="/indonesia/raja-ampat" style={{ display: "block", fontSize: 12, color: "#60a5fa", marginBottom: "1rem" }}>View all sites in Raja Ampat</Link>
        ...
        <div style={{ overflowY: "auto" }}>
          <div style={{ background: "#020d1a url(/hero-bg.png) center top / cover no-repeat", padding: "1.25rem 2rem", borderBottom: "1px solid #e2e8f0" }}>
            <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 4 }}>
              <Link href="/" style={{ color: "#475569" }}>Home</Link> › <Link href="/indonesia" style={{ color: "#475569" }}>Indonesia</Link> › <Link href="/indonesia/raja-ampat" style={{ color: "#475569" }}>Raja Ampat</Link>
            </div>
            <div style={{ fontSize: 20, fontWeight: 900, color: "#fff" }}>RAJA AMPAT</div>
          </div>
          ...
          <div style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0", padding: "0.75rem 2rem", display: "flex", gap: 8, overflowX: "auto" }}>
            {RAJA_AMPAT_AREAS.map(a => (
              <Link key={a.slug} href={`/indonesia/raja-ampat/areas/${a.slug}`}
          ...
          <div style={{ padding: "1.5rem 2rem", display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.5rem", alignItems: "start" }}>
            <div>
              <div style={{ marginBottom: "1.5rem" }}>
                <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                  <span style={{ fontSize: 11, padding: "3px 10px", background: "#0a1628", color: "#fff", borderRadius: 4, fontWeight: 700 }}>#{site.rank} in Raja Ampat</span>
                  <span style={{ fontSize: 11, padding: "3px 10px", background: "#dbeafe", color: "#1d4ed8", borderRadius: 4, fontWeight: 600 }}>Admin verified</span>
                </div>
                <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 6 }}>Indonesia › Raja Ampat › {site.area}</div>
                ...
                <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
                  <span style={{ fontSize: 13, color: "#64748b" }}>Location: {site.area}, Raja Ampat, West Papua, Indonesia</span>
                </div>
            ...
            <div>
              <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "1.25rem", marginBottom: 12 }}>
                ...
                {([["Location", site.area + ", Raja Ampat"], ["Type", site.type], ["Access", site.access], ["Depth", site.minDepth + "m – " + site.maxDepth + "m"], ["Visibility", site.visibility], ["Water temp", site.temp], ["Difficulty", site.difficulty], ["Best time", site.bestTime]] as [string,string][]).map(([k, v]) => (
              ...
              {nearby.length > 0 && (
                <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "1.25rem" }}>
                  ...
                  {nearby.map(s => (
                    <Link key={s.slug} href={`/indonesia/raja-ampat/${s.areaSlug}/${s.slug}`} style={{ display: "flex", gap: 10, padding: "8px 0", borderBottom: "1px solid #f8fafc", alignItems: "center", textDecoration: "none" }}>
