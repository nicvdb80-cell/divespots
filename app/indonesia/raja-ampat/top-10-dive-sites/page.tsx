...
import { RAJA_AMPAT_SITES } from '@/lib/data'

export const metadata = {
  title: 'Top 10 Raja Ampat Dive Sites | Dive Spots',
  description: 'The definitive guide to the top 10 dive sites in Raja Ampat, West Papua, Indonesia — ranked by our expert team and diver community.',
}

export default function RajaAmpatTop10Page() {
  return (
      ...
      <div style={{background:'linear-gradient(135deg,#0a1628,#1e3a5f)',padding:'3rem 2rem',color:'#fff',borderBottom:'1px solid #1e3a5f'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div style={{fontSize:11,color:'#60a5fa',marginBottom:8}}>
            <Link href="/destinations" style={{color:'#475569'}}>Destinations</Link> › <Link href="/indonesia" style={{color:'#475569'}}>Indonesia</Link> › <Link href="/indonesia/raja-ampat" style={{color:'#475569'}}>Raja Ampat</Link> › Top 10
          </div>
          <h1 style={{fontSize:36,fontWeight:900,letterSpacing:-1,marginBottom:10}}>TOP 10 RAJA AMPAT DIVE SITES</h1>
          <p style={{color:'#64748b',fontSize:14,lineHeight:1.7,maxWidth:540}}>The definitive ranking of Raja Ampat&apos;s best dive sites — selected and verified by our expert team and diver community.</p>
        </div>
      </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {RAJA_AMPAT_SITES.map(site=>(
            <Link key={site.slug} href={`/indonesia/raja-ampat/${site.areaSlug}/${site.slug}`}
              style={{display:'flex',alignItems:'center',gap:16,padding:'1.25rem 1.5rem',border:'1px solid #f1f5f9',borderRadius:12,background:'#fff',textDecoration:'none'}}>
