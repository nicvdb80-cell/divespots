...
export const metadata = {
  title: 'Raja Ampat Dive Sites | West Papua, Indonesia | Dive Spots',
  description: 'Explore dive sites in Raja Ampat, West Papua — the most biodiverse coral reef system on earth. Mantas, sharks, pristine walls, and record-breaking fish counts.',
}
  ...
  return (
    <main style={{ minHeight:'100vh', background:'#0d1b2e' }}>
      ...
      <div style={{ background:'#020d1a url(/hero-bg.png) center center / cover no-repeat', padding:'2.5rem 2rem', color:'#fff', borderBottom:'1px solid #1e3a5f' }}>
        ...
        <p style={{ color:'#64748b',maxWidth:560,fontSize:13,lineHeight:1.7,marginBottom:'1.5rem' }}>Raja Ampat is the epicentre of global marine biodiversity — home to the world&apos;s most biodiverse coral reef system, record-breaking fish counts, oceanic mantas, and pristine walls stretching into the deep blue.</p>
        <div style={{ display:'flex',gap:'2.5rem' }}>
          {[['200+','Dive sites'],['27–30°C','Water temp'],['40m+','Visibility'],['Oct–Apr','Best season']].map(([n,l])=>(
      ...
      <div style={{ background:'#0a1628',borderBottom:'1px solid #1e3a5f',padding:'0.75rem 2rem',display:'flex',gap:8,overflowX:'auto' }}>
        {RAJA_AMPAT_AREAS.map(a=>(
          <Link key={a.slug} href={`/indonesia/raja-ampat/areas/${a.slug}`} style={{ padding:'8px 16px',background:'#0d1b2e',border:'1px solid #1e3a5f',borderRadius:8,fontSize:11,fontWeight:600,color:'#94a3b8',whiteSpace:'nowrap',flexShrink:0,textDecoration:'none' }}>
      ...
      <div style={{ maxWidth:1100,margin:'0 auto',padding:'2rem',display:'grid',gridTemplateColumns:'240px 1fr',gap:'2rem' }}>
        ...
        <div>
          ...
          <div style={{ marginTop:'1.5rem',textAlign:'center' }}>
            <Link href="/indonesia/raja-ampat/top-10-dive-sites" style={{ fontSize:13,color:'#60a5fa',fontWeight:600 }}>View full Top 10 list →</Link>
          </div>
        </div>
      </div>
