  ...
  return (
    <main style={{ minHeight:'100vh', background:'#0d1b2e' }}>
      ...
      <div style={{ background:'#020d1a url(/hero-bg.png) center center / cover no-repeat', padding:'2.5rem 2rem', color:'#fff', borderBottom:'1px solid #1e3a5f' }}>
        <div style={{ fontSize:11,color:'#60a5fa',marginBottom:8 }}>🇮🇩 Indonesia · Borneo (Kalimantan)</div>
        <h1 style={{ fontSize:36,fontWeight:900,marginBottom:10 }}>BORNEO DIVE SITES</h1>
        <p style={{ color:'#64748b',maxWidth:560,fontSize:13,lineHeight:1.7,marginBottom:'1.5rem' }}>The Derawan Archipelago in East Kalimantan is the second most biodiverse marine area on Earth. Year-round manta rays at Sangalaki, thresher sharks at Kakaban&apos;s dramatic walls, stingless jellyfish in an ancient lake, and Borneo&apos;s finest macro diving at Derawan&apos;s famous jetty.</p>
        <div style={{ display:'flex',gap:'2.5rem' }}>
          {[['90+','Dive sites'],['27–30°C','Water temp'],['40m+','Visibility'],['Year round','Diving']].map(([n,l])=>(
      ...
      <div style={{ maxWidth:1100,margin:'0 auto',padding:'2rem',display:'grid',gridTemplateColumns:'240px 1fr',gap:'2rem' }}>
        <div>
          <div style={{ background:'#0a1628',border:'1px solid #1e3a5f',borderRadius:12,padding:'1.25rem',marginBottom:12 }}>
            ...
            {BORNEO_AREAS.map(a=>(
              <Link key={a.slug} href={`/indonesia/borneo/areas/${a.slug}`} style={{ padding:'8px 0',borderBottom:'1px solid #1e3a5f',display:'flex',justifyContent:'space-between',alignItems:'center',textDecoration:'none' }}>
                <div>
        ...
        <div>
          <h2 style={{ fontSize:18,fontWeight:800,color:'#fff',marginBottom:'1.25rem' }}>TOP DIVE SITES IN BORNEO</h2>
          <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
            {BORNEO_SITES.map(site=>(
