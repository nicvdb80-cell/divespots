import { BORNEO_SITES, BORNEO_AREAS, getBorneoSiteBySlug } from '@/lib/data'
...
export async function generateMetadata({ params }: { params: Promise<{ area: string; slug: string }> }) {
  ...
  const site = getBorneoSiteBySlug(slug)
  if (!site) return {}
  return {
    title: `${site.name} Dive Site Guide | ${site.area}, Borneo, Indonesia | Dive Spots`,
    description: `Complete diver guide to ${site.name} in ${site.area}, Sulawesi. Depth ${site.minDepth}-${site.maxDepth}m.`,
  }
}

export default async function BorneoDiveSitePage({ params }: { params: Promise<{ area: string; slug: string }> }) {
  const { slug } = await params
  const site = getBorneoSiteBySlug(slug)
  if (!site) { notFound(); return null }

  const sidebarSites = BORNEO_SITES.filter(s => s.areaSlug === site.areaSlug)
  const nearby = BORNEO_SITES.filter(s => s.areaSlug === site.areaSlug && s.slug !== site.slug).slice(0, 3)
      ...
      <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", minHeight: "calc(100vh - 60px)" }}>
        <aside style={{ background: "#0a1628", borderRight: "1px solid #1e3a5f", overflowY: "auto", maxHeight: "calc(100vh - 60px)", position: "sticky", top: 60 }}>
          <div style={{ padding: "1rem", borderBottom: "1px solid #1e3a5f" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 800, color: "#fff" }}>SULAWESI · {site.area.toUpperCase()}</div>
          </div>
          <div style={{ padding: "0.5rem 0" }}>
            {sidebarSites.map(s => (
              <Link key={s.slug} href={`/indonesia/borneo/${s.areaSlug}/${s.slug}`}
          ...
          <div style={{ borderTop: "1px solid #1e3a5f", padding: "1rem" }}>
            <Link href="/indonesia/borneo" style={{ display: "block", fontSize: 12, color: "#60a5fa", marginBottom: "1rem" }}>← All Borneo sites</Link>
          </div>
        </aside>
        ...
        <div style={{ overflowY: "auto" }}>
          <div style={{ background: "#020d1a", padding: "1.25rem 2rem", borderBottom: "1px solid #1e3a5f" }}>
            <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 4 }}>
              <Link href="/" style={{ color: "#475569" }}>Home</Link> › <Link href="/indonesia" style={{ color: "#475569" }}>Indonesia</Link> › <Link href="/indonesia/borneo" style={{ color: "#475569" }}>Sulawesi</Link> › {site.area}
            </div>
          ...
          <div style={{ padding: "1.5rem 2rem", display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.5rem", alignItems: "start" }}>
            <div>
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 6 }}>Indonesia › Borneo › {site.area}</div>
            ...
            <div>
              <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "1.25rem", marginBottom: 12 }}>
                ...
                {([["Location", site.area + ", Borneo"], ["Type", site.type], ["Access", site.access], ["Depth", site.minDepth + "m – " + site.maxDepth + "m"], ["Visibility", site.visibility], ["Water temp", site.temp], ["Difficulty", site.difficulty], ["Best time", site.bestTime]] as [string,string][]).map(([k, v]) => (
              ...
              {nearby.length > 0 && (
                <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: "1.25rem" }}>
                  ...
                  {nearby.map(s => (
                    <Link key={s.slug} href={`/indonesia/borneo/${s.areaSlug}/${s.slug}`} style={{ display: "flex", gap: 10, padding: "8px 0", borderBottom: "1px solid #f8fafc", alignItems: "center", textDecoration: "none" }}>
