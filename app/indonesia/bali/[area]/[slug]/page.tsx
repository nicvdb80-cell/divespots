...
export async function generateMetadata({ params }: { params: { area: string; slug: string } }) {
  ...
  return {
    ...
    description: `Complete diver guide to ${site.name} in ${site.area}, Bali. Depth ${site.minDepth}–${site.maxDepth}m, visibility ${site.visibility}, ${site.access.toLowerCase()} access.`,
  }
}
        ...
        <aside style={{ background: '#0a1628', borderRight: '1px solid #1e3a5f', overflowY: 'auto', maxHeight: 'calc(100vh - 60px)', position: 'sticky', top: 60 }}>
          ...
          <div style={{ borderTop: '1px solid #1e3a5f', padding: '1rem' }}>
            ...
            <div style={{ background: '#0d1b2e', borderRadius: 10, padding: '0.875rem' }}>
              ...
              <Link href="/submit-dive-site" style={{ display: 'block', background: '#16a34a', color: '#fff', padding: '7px 10px', borderRadius: 7, fontSize: 11, fontWeight: 700, textAlign: 'center', marginTop: 8 }}>Submit a dive site</Link>
            </div>
          </div>
        </aside>
        ...
        <div style={{ overflowY: 'auto' }}>
          <div style={{ background: 'linear-gradient(135deg,#0a1628,#1e3a5f)', padding: '1.25rem 2rem', borderBottom: '1px solid #e2e8f0' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 900, color: '#fff' }}>BALI <span style={{ fontSize: 13, color: '#60a5fa', fontWeight: 400 }}>Indonesia</span></div>
            </div>
            <div style={{ display: 'flex', gap: '2rem', marginTop: '0.75rem' }}>
              {[['120+', 'Dive sites'], ['27–29°C', 'Water temp'], ['15–40m+', 'Max depth'], ['Apr – Nov', 'Best season']].map(([n, l]) => (
                <div key={l}><div style={{ fontSize: 14, fontWeight: 800, color: '#fff' }}>{n}</div><div style={{ fontSize: 10, color: '#475569', textTransform: 'uppercase', letterSpacing: 1 }}>{l}</div></div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0', padding: '0.75rem 2rem', display: 'flex', gap: 8, overflowX: 'auto' }}>
            {BALI_AREAS.map(a => (
              <Link key={a.slug} href={`/indonesia/bali/areas/${a.slug}`}
                ...
                {a.name} <span style={{ fontSize: 9, color: site.areaSlug === a.slug ? '#60a5fa' : '#94a3b8' }}>{a.count}</span>
              </Link>
            ))}
          </div>
          ...
          <div style={{ padding: '1.5rem 2rem', display: 'grid', gridTemplateColumns: '1fr 300px', gap: '1.5rem', alignItems: 'start' }}>
            ...
            <div>
              <div style={{ background: 'linear-gradient(135deg,#0a1628,#1e3a5f)', borderRadius: 12, height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 11, color: '#60a5fa', marginBottom: 4 }}>{site.area} · {Math.abs(site.lat).toFixed(4)}° S</div>
                  <Link href="/map" style={{ fontSize: 11, color: '#60a5fa' }}>View on map →</Link>
                </div>
              </div>
              ...
              <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 12, padding: '1.25rem', marginBottom: 12 }}>
                ...
                <div style={{ fontSize: 12, color: '#166534', marginBottom: 3 }}>Verified: Local dive professionals, {site.area}</div>
